...
const makeUsers = (count) => Array.from({ length: count }, (_, i) => ({
    id: i,
    name: `User ${i}`,
    age: i % 100,
    tags: ["a", "b", "c"],
...
const users10k = makeUsers(10000);
// FunState fixtures
const userState = funState({ ...baseUser });
const ageState = userState.prop("age");
const nameState = userState.prop("name");
const derivedLabel = derive(nameState, ageState, (name, age) => `${name}-${age}`);
const mappedAge = mapRead(ageState, (age) => age + 1);
const mappedLabel = mapRead(derivedLabel, (label) => label.toUpperCase());
const listState = funState(users1000);
const listItemState = listState.focus(Acc().at(500));
