 ...
 * @sig (b -> c) -> (a -> b) -> a -> c
 */
export const B = <B, C>(g: (y: B) => C) => <A>(f: (x: A) => B) => (x: A): C => g(f(x))
 ...
 */
export const prop = <Obj>() => <K extends keyof Obj>(k: K): Accessor<Obj, Obj[K]> => ({
 ...
 */
export const set = <S, A>(acc: Accessor<S, A>): ((x: A) => (s: S) => S) => B(acc.mod)(K)
